...
random_data_surv = function(N,lambda=1/2,q=1,of=0) {
...
folder_= '../../result/simulation_data/4_X_clayton_simulation'
...
for(loop in 1:100){
  ...
  for(i in 1:2){
    for(j in (i+1):3){
      ...
      TTE = cbind(RECOV.EVENT[,2*i-1],RECOV.EVENT[,2*i],RECOV.EVENT[,2*j-1],RECOV.EVENT[,2*j]);
      TTE[TTE[,2]==1,2] = TTE[TTE[,2]==1,1] + epsilon;
      TTE[TTE[,2]==0,2] = MAX
      TTE[TTE[,4]==1,4] = TTE[TTE[,4]==1,3] + epsilon;
      TTE[TTE[,4]==0,4] = MAX + epsilon;
      mle = computeMLE(R=TTE, B=c(1,1,1,1));
      SS = NULL;
      ...
      ifelse((i==1 & j==2), prAB_values_mle <- prAB_value_mle, prAB_values_mle <- cbind(prAB_values_mle, prAB_value_mle))
    }
  }
  ...
  rownames(prAB_values_mle) = c(loop)
...
colnames(prAB_result_mle) = c("X1 & X2", "X1 & X3", "X2 & X3")
...
write.csv(prAB_result_mle, file=sprintf("%s/prAB_result_mlecens.csv", folder_))
